    ...
    <section className="space-y-3">
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#1a1a1a]/35" />
          <input
            type="search"
            value={q}
            onChange={(e) => onSearchChange(e.target.value)}
            placeholder="Rechercher par email..."
            className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-white border border-black/[0.08] focus:border-[#d4a843]/40 focus:outline-none text-[14px] placeholder:text-[#1a1a1a]/30"
          />
        </div>
        <a
          href={exportHref}
                ...
                <td className="px-4 py-3 text-right">
                  {row.status === 'active' && (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[12px] font-semibold text-red-600 hover:bg-red-50 transition"
                    >
        ...
        {rows.map((row) => (
          ...
          >
            ...
            {row.status === 'active' && (
              <button
                onClick={() => onUnsubscribe(row)}
                className="mt-3 w-full inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[12px] font-semibold text-red-600 bg-red-50 hover:bg-red-100 transition"
              >
                <UserMinus className="w-3.5 h-3.5" />
                Désabonner
              </button>
