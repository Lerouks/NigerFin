...
export async function GET() {
  ...
  const [presentationRes, factsRes, regionsRes, resourcesRes, indicatorsRes, partnersRes] = await Promise.all([
    ...
    supabase.from('niger_partners').select('*').order('display_order', { ascending: true }),
  ]);
