...
function PlanCard({ title, badge, tools, isPremium = false }: PlanCardProps) {
  return (
    <div className={`rounded-xl overflow-hidden border transition-all duration-300 ${
      ...
      <div className={`px-6 pt-6 pb-5 ${isPremium ? 'border-b border-white/[0.06]' : 'border-b border-black/[0.05]'}`}>
        <span className={`inline-block text-[10px] tracking-[0.15em] uppercase mb-3 ${isPremium ? 'text-white/40' : 'text-gray-400'}`}>
          {badge}
        </span>
        <h3 className={`text-[22px] ${isPremium ? 'text-white' : ''}`}>{title}</h3>
      </div>
        ...
        {tools.map((tool) => (
          ...
          >
            ...
            <div className="flex-1">
              <p className={`text-[14px] ${isPremium ? 'text-white/90' : 'text-gray-700'}`}>{tool.name}</p>
            </div>
            <ArrowRight className={`w-4 h-4 opacity-0 -translate-x-1 group-hover/item:opacity-100 group-hover/item:translate-x-0 transition-all ${
    ...
    <section id="outils" className="py-20 md:py-28">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-xl mb-14">
          <span className="text-[11px] tracking-[0.15em] uppercase text-gray-400 block mb-3">Outils financiers</span>
          <h2 className="text-3xl md:text-4xl mb-4">Calculez, simulez, anticipez</h2>
          <p className="text-gray-500 text-[15px] leading-relaxed">
            Des calculateurs et simulateurs professionnels conçus pour les
            investisseurs et professionnels au Niger et en Afrique.
          </p>
        </div>
