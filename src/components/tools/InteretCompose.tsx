...
export function InteretCompose() {
  ...
  const analysis = useMemo(() => {
    ...
    lines.push(
      `Un capital de ${fmt(numCapital)} FCFA place a ${numRate}% avec capitalisation ${COMPOUNDS.find(c => c.value === compoundsPerYear)?.label.toLowerCase()} pendant ${numYears} an${numYears > 1 ? 's' : ''} atteindra ${fmt(calc.total)} FCFA.`
    );

    lines.push(
      `Les interets composes s'elevent a ${fmt(calc.interest)} FCFA, contre ${fmt(calc.simpleInterest)} FCFA en interet simple. L'effet de la capitalisation apporte un bonus de ${fmt(calc.bonus)} FCFA.`
    );

    if (numYears >= 10) {
      lines.push('Sur une duree longue, l\'effet des interets composes est significatif. Le capital beneficie pleinement de l\'effet \"boule de neige\" de la capitalisation.');
    } else if (numYears >= 5) {
      lines.push('La duree est suffisante pour que l\'effet des interets composes se manifeste de maniere notable.');
    } else {
      lines.push('Sur une courte duree, la difference entre interets simples et composes reste moderee. L\'effet se renforce avec le temps.');
    }
  ...
  return (
    <div className="space-y-8">
      ...
      {!hasInput ? (
        <EmptyState icon={TrendingUp} message="Renseignez le capital, le taux et la duree pour calculer les interets composes." />
      ) : calc ? (
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-[#111] text-white p-6 rounded-xl">
              <p className="text-[12px] text-white/40 uppercase tracking-wider mb-1">Interets composes</p>
            ...
            <div className="bg-white border border-black/[0.06] p-6 rounded-xl">
              <p className="text-[12px] text-gray-400 uppercase tracking-wider mb-1">Bonus vs interet simple</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <ResultSummary
              items={[
                { label: 'Interets composes', value: `${fmt(calc.interest)} FCFA`, highlight: true },
                { label: 'Montant final', value: `${fmt(calc.total)} FCFA` },
                { label: 'Interets simples (comparaison)', value: `${fmt(calc.simpleInterest)} FCFA` },
